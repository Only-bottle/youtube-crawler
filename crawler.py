...
load_wb = openpyxl.load_workbook('./data/data.xlsx')
print(type(load_wb))
print(load_wb.get_sheet_names())
...
# print(load_ws['A1':'B318'])
for row in load_ws['A1':'B318']:
    row_value = []
    for cell in row:
...
for row in range(1):  # 0~149 까지 엑셀파일의 모든 title loop
    # HTTP request
    ...
    while True:
        ...
        print(running_time, splitted_time)
        ...
        if len(splitted_time) == 2:  # 재생시간이 분, 초
            if int(splitted_time[0]) >= 7:  # 재생시간이 7분 이상이면 다음 순서의 동영상을 가져온다.
                ...
                watch_url = soup.find_all(class_='yt-uix-tile-link yt-ui-ellipsis yt-ui-ellipsis-2 yt-uix-sessionlink spf-link ')[0]['href']
                break
    ...
    # 유튜브 가져오기
    youtube = YouTube("https://www.youtube.com" + watch_url)  # 동영상 url
    #     print(youtube.title)
    #     print(youtube)
    ...
    parent_dir = "C:/Users/ssey0/OneDrive/바탕 화면/Graduation project/Youtude_crawler/mp3"  # 저장할 파일 경로
